...
# produce the scatterplot using ggplot()
scatterplot_longevity_brainsize <- ggplot(d, aes(x = Brain_Size_Species_Mean, y = MaxLongevity_m)) + 
  geom_point() +  # graph the points
  geom_smooth(method = "lm", formula = y ~ x, color = "orange", se = FALSE) +  # Add the fitted line
  geom_text(aes(label = paste("y = ", round(coef(model1)[1], 2), " + ", round(coef(model1)[2], 2), "x")), x = min(d$Brain_Size_Species_Mean), y = max(d$MaxLongevity_m),  hjust = 0, vjust = 1, color = "orange") +  # write the equation for the fitted line
  labs(title = "Scatterplot: Longevity ~ Brain Size", x = "Brain Size (grams)", y = "Longevity (months)")
